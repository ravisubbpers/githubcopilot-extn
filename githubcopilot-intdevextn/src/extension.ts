 ...
 */
class RkIdpViewProvider implements vscode.WebviewViewProvider {
	
	public static readonly viewType = 'rk-idp-main';
	...
	private async _initStorage(): Promise<void> {
		try {
			...
			const storageDir = path.join(require('os').homedir(), '.vscode-chat-exports');
	 ...
	 */
	private async _saveChatToStorage(fileName: string, jsonContent: string, parsedJson: any): Promise<void> {
		try {
			const fs = require('fs');
			...
			const existingExports = this._context.globalState.get<any[]>('chatExports', []);
			...
			const path = require('path');
			const backupDir = path.join(require('os').homedir(), '.vscode-chat-exports');
	 ...
	 */
	private async _extractAndSaveMetrics(exportId: string, parsedJson: any): Promise<void> {
		try {
	...
	private async _clearChatExports(): Promise<void> {
		try {
						...
						try {
							...
							for (const key of sessionKeys) {
								...
								if (sessionData && typeof sessionData === 'object' && 'metrics' in sessionData) {
									...
									const sessionTimestamp = (sessionData as any).sessionTimestamp || (sessionData as any).timestamp;
									const formattedDate = new Date(sessionTimestamp).toLocaleDateString('en-GB'); // dd/mm/yyyy format
									
									for (const metric of (sessionData as any).metrics) {
										allMetrics.push({
											sessionId,
											date: formattedDate,
	...
	private _getConnectionStatus(value: string): { status: string, color: string } {
		if (!value || value.trim() === '') {
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(RkIdpViewProvider.viewType, provider)
	);
